...
class ParallelizeTest(unittest.TestCase):
    ...
    def test_result_correctness_list(self):
        self.assertEqual(parallelize([], 3, self._foo_l),
                         [1, 2, 3, 1, 2, 3, 1, 2, 3])

    def test_result_correctness_dict(self):
        self.assertEqual(parallelize([], 2, self._foo_d),
                         [1, 2, 1, 2])

    def test_result_correctness_string(self):
        self.assertEqual(parallelize([], 2, self._foo_s),
                         ['a', 'b', 'c', 'a', 'b', 'c'])
